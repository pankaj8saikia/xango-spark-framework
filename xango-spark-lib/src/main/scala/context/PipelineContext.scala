...
class PipelineContext {
  ...
  private var processingBatch: Option[ProcessingBatch] = None
  ...
  def setCurrentBatch(value: ProcessingBatch): ProcessingBatch = {
    processingBatch = Some(value)
    value
  }

  def currentBatch: ProcessingBatch =
    processingBatch.getOrElse(
      throw new IllegalStateException("Processing batch was not initialized")
    )

  def currentBatchOption: Option[ProcessingBatch] = processingBatch

  def clearCurrentBatch(): Unit = {
  ...
  def clearDataFrames(): Unit = dataFrames.clear()
}
